...
export class TimeoutPromise<V> implements Promise<V> {
  ...
  then <V1 = V, V2 = never> (
    ...
    onrejected?: ((reason: any) => VV | PromiseLike<VV>
    ) | undefined | null): TimeoutPromise<V | VV> {
...
export function setTimeout (duration: number): TimeoutPromise<undefined>;
export function setTimeout <V>(fn: V, duration: number): TimeoutPromise<Exec<V>>;
export function setTimeout (fn?: any, duration?: number): TimeoutPromise<any> {
  if (typeof fn === 'number') {
    duration = fn
    fn = undefined
  }
  ...
  const timeoutPromise = new TimeoutPromise((resolve, reject) => {
    const timeoutHandler = (): void => {
      exec(fn, timeoutPromise).then(resolve).catch(reject)
    }
    timeoutData = {
      resolve,
      reject,
