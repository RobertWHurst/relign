...
export class IntervalPromise<V> implements Promise<V> {
  ...
  then <V1 = V, V2 = never> (
    ...
    const p = new IntervalPromise<V1 | V2>(this._promise.then(onfulfilled, onrejected))
    ...
    const p = new IntervalPromise<V | VV>(this._promise.catch(onrejected))
    p._intervalData = this._intervalData
    return p
  }
...
export function setInterval<V> (fn: () => void, duration: number): IntervalPromise<V> {
  ...
  const intervalPromise = new IntervalPromise<V>((resolve, reject) => {
    ...
    const intervalId = global.setInterval(intervalHandler, duration as number)
    intervalData = {
      resolve,
      reject,
      intervalHandler,
